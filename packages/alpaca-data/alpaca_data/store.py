...
DEFAULT_DB = Path(
    os.environ.get(
        "ALPACA_DATA_DB",
        Path.home() / ".local" / "share" / "finance-tools" / "alpaca.sqlite3",
    )
).expanduser()
...
class Alpaca:
    """Small reusable Alpaca REST client used by the persistence helpers."""
    ...
    def __init__(self, key_id: str | None = None, secret_key: str | None = None) -> None:
        if key_id is None and secret_key is None:
            ...
            raise ValueError("key_id and secret_key must be provided together")
