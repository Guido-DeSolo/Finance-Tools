...
This directory contains the non-visual parts of DF-FinTechTerm. Keep supervised
workers, user actions, and shared deterministic logic separate:

```text
DF-FinTechTerm terminal / Finance Shell
              |
              +--> actions  -> finite user-requested research
              |
              +--> services -> ingestion and scheduled scoring workers
                                      |
                                      +--> PostgreSQL / Alpaca / OpenInsider
```

From the application root, use `./run.sh services` and `./run.sh actions` to
inspect the two catalogs. `./run.sh service NAME` runs a worker in the
foreground, making it suitable for systemd or another supervisor. `./run.sh
action NAME` runs a finite interactive or research operation. `./run.sh catalog`
provides the same boundary as JSON for UI viewports.

The supplied `systemd/df-fintechterm@.service` template keeps each worker as a distinct
supervised unit. For example, after linking it into the user unit directory,
`systemctl --user start df-fintechterm@news-ingest.service` runs only the news worker.
The unit defaults to backtest mode and reads optional local configuration from
`~/.config/df-fintechterm/df-fintechterm.env`; that private file is never part of this repository.

`DF_FINTECHTERM_MODE` accepts `backtest`, `shadow`, `paper`, or `live` and defaults to
`backtest`. No backend module currently places orders. Future execution code
must use the runtime guard in `fintech_core/runtime.py` and require a separate
