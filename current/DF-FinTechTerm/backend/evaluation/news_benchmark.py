#!/usr/bin/env python3
"""Run the frozen NEWS specialist benchmark without persistence."""
...
def main():
    parser = argparse.ArgumentParser(description="Run frozen NEWS benchmark v1.")
    parser.add_argument("--model", default=news.DEFAULT_MODEL)
    parser.add_argument("--base-url")
    parser.add_argument("--timeout", type=float, default=900)
    parser.add_argument("--output", required=True, type=Path)
    args = parser.parse_args()
    settings = load_settings()
    base_url = args.base_url or settings.get("OLLAMA_BASE_URL", news.DEFAULT_OLLAMA_URL)
    ...
    for symbol in corpus["symbols"]:
        ...
        try:
            result, raw, invoked = news.analyze_news(summary, base_url, args.model, args.timeout)
            entry = {
