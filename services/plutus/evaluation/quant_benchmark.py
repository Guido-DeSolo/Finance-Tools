#!/usr/bin/env python3
"""Run the frozen five-case QUANT benchmark without persistence."""
...
CORPUS_PATH = Path(__file__).with_name("quant_corpus.json")


def main():
    parser = argparse.ArgumentParser(description="Run frozen QUANT benchmark v1.")
    parser.add_argument("--model", default=quant.DEFAULT_MODEL)
    parser.add_argument("--base-url")
    parser.add_argument("--timeout", type=float, default=900)
    parser.add_argument("--output", required=True, type=Path)
    args = parser.parse_args()
    settings = load_settings()
    base_url = args.base_url or settings.get("OLLAMA_BASE_URL", quant.DEFAULT_OLLAMA_URL)
    ...
    for symbol in corpus["symbols"]:
        ...
        try:
            result, raw, invoked = quant.analyze_market(
            ...
            entry = {
                "status": "rejected",
                "model_invoked": summary["market_available"],
                "elapsed_seconds": round(time.monotonic() - started, 3),
                "error": str(exc),
            }
