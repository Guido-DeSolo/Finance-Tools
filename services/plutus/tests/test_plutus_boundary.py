...
from plutus_cli import ACTIONS, SERVICES, catalog_payload
from plutus_core import ExecutionMode, RuntimeConfig
...
class RuntimeBoundaryTests(unittest.TestCase):
    ...
    def test_cli_lists_both_surfaces(self):
        services = subprocess.run(
            [sys.executable, str(ROOT / "plutus_cli.py"), "services"],
            check=True, capture_output=True, text=True,
        )
        actions = subprocess.run(
            [sys.executable, str(ROOT / "plutus_cli.py"), "actions"],
            check=True, capture_output=True, text=True,
        )
